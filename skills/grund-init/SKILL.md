...
- The installed default asks agents for plain local citations. Use `--conversation link` as the user-level override when the TUI has no rendering support. Neither the preference nor the editor choice (`GRUND_OPEN_CMD` or `EDITOR`) belongs in shared repo config; repository instructions carry only the fixed repository-web rule.
