...
- One opinion *is* committable: a repository may set `[reference] conversation = "link"` in `.agents/grund.toml` (§DF-repo-conversation-opinion), and the generated entrypoint then teaches linked local citations — `§<ID>` followed by its location as plain `path:line` text — to every cloner with zero per-user setup. It takes precedence over the user preference inside that repository; without it, repository instructions carry only the fixed repository-web rule. Offer it when a team wants clickable conversation citations without asking each member to run `grund integrations`.
