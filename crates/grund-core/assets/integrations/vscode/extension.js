...
function resolve(id, cwd) {
  ...
  return new Promise((done) => {
    execFile('grund', [id, '--brief', '--format', 'json'], { cwd }, (err, stdout) => {
      let value = null;
      if (!err) {
        try { value = JSON.parse(stdout); } catch { value = null; }
      }
      // Cache only successes. A failure is not a fact about the citation: the
      // declaration may not exist *yet*, or grund may be transiently missing
      // from PATH — caching null would pin that state until the extension host
      // restarts, long after the user fixed the cause.
  ...
  const provider = vscode.window.registerTerminalLinkProvider({
    async provideTerminalLinks(ctx) {
      ...
      const decls = await Promise.all(
        found.map((f) => resolve(f.text.replace(/^[^A-Za-z0-9]+/, ''), cwd))
      );
    ...
    async handleTerminalLink(link) {
      ...
      if (decl.line === undefined) {
        ...
        vscode.commands.executeCommand(inWorkspace ? 'revealInExplorer' : 'revealFileInOS', abs);
        return;
      }
      const pos = new vscode.Position(Math.max(0, decl.line - 1), 0);
