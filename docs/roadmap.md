...
The e2e goldens carry the release as literal bytes and a unit test asserts it is still ahead of `CARGO_PKG_VERSION`, so the version bump that reaches the deadline fails CI — the same forcing function [§RM-index-entry-error](roadmap.md#rm-index-entry-error-flip-the-missing-index-entry-warning-to-an-error) uses and the one that carried the `[[kinds]] prefix` removal ([§FS-config.3.4.6](functional-spec/FS-config.md#346-prefix-the-former-spelling-of-kind-removed-in-0130)) to its release, for the same reason: a deadline that can pass quietly is not a deadline.
