...
- A home marked `scan = false` ([§FS-config.3.4.7](../functional-spec/FS-config.md#347-scan--a-place-that-is-listed-not-walked)) is skipped, and this rule is not a name rule either: it strips the entry's in-tree path to the config root and compares it against the home, the way §2.4 decides which home a file is in, so it prunes *that* directory and not every directory sharing its last component. Leaving such a home out of the root list below is only half of it — the walk meets the same directory again as a descendant of any root above it — and the rule is skipped entirely under `--full`, which reaches it like any unconfigured directory, and in a tree that configures no unwalked kind, which is nearly all of them ([§GOAL-fast-feedback](../goals.md#goal-fast-feedback-grund-must-be-as-fast-as-possible)).
...
| Python                   | `""" … """` or `''' … '''` docstring                 | special-cased (see note 4.2)         |
