...
- A citation inside a source comment block also carries that block as an inline citation site ([§FS-inline-citation-style.1](../functional-spec/FS-inline-citation-style.md#1-scope)): its first and last line, its widest column, whether it carries a note, and the ascending list of its citation-bearing lines that fail the configured `[reference] inline_note_layout` ([§FS-inline-citation-style.3.3](../functional-spec/FS-inline-citation-style.md#33-inline_note_layout--where-the-citations-sit)). The layout list is what lets the checker report a per-line deviation without re-reading the file; it is empty, and no line is classified, at the default `inline_note_layout = "any"`, under `inline_style = "citation-only"`, and at `inline_note_layout_check = "off"`, where the verdicts would reach no channel ([§FS-inline-citation-style.4.4](../functional-spec/FS-inline-citation-style.md#44-warnings-and-errors--opt-in-layout-deviations)) — so a project that configures no layout, or configures one without gating it, tokenizes no line and classifies no line on the field's account (one empty per-block memo is the whole cost). Every citation in one block carries the same site.
