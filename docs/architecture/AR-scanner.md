...
The roots the walk starts from are `[scan] include` resolved against the config root, or an explicit path argument when one is given, or — under `grund check --full` ([§FS-check.1.3](../functional-spec/FS-check.md#13-the-full-tree-scope---full)) — the config root itself, which is `include` cancelled and nothing more. The skip rules above, the ignore files, and the extension filter below apply identically to all three, so the only thing that varies between them is where the descent begins.
