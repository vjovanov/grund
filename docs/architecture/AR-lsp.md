...
Which snapshot that is comes from one resolution used by every surface, requests and diagnostics alike, so an editor cannot navigate against one project's view while reading another's errors. A project whose root contains the document owns it, the deepest root winning among nested trees; failing that, a project whose scan reached the document owns it. The second half is why `LspSnapshot` carries the set of files its scan read: a project's reach is not bounded by its root — a symlinked `[scan] include` canonicalizes outside it, and an include path may be parent-relative — so a root prefix alone would answer "not mine" for files the CLI checks, and the editor would go quiet on exactly the citations [§FS-lsp.1](../functional-spec/FS-lsp.md#1-what-the-server-provides) exists to surface. Diagnostics are then published by the owner alone; the same finding reaching two snapshots is reported once.
