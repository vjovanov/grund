...
| A block that claims nothing here is never expanded | `an_ancestor_that_claims_nothing_here_cannot_break_the_run`, `an_ancestor_with_overlapping_members_that_claims_nothing_is_climbed_past`, `an_ancestor_glob_claims_the_child_and_still_owes_it_an_answer`, `an_ancestor_claim_through_a_symlinked_entry_keeps_the_prefix`, `an_ancestor_that_does_not_load_and_claims_nothing_here_is_climbed_past` (`crates/grund-core/src/tests_workspace_claims.rs`) |
| An undecidable claim warns and lets the run through | `an_ancestor_whose_members_text_cannot_be_read_warns_and_lets_the_run_through`, `an_ancestor_config_that_is_not_text_is_reported_and_climbed_past` (`crates/grund-core/src/tests_workspace_claims.rs`) |
