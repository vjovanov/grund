...
That next release is `0.13.0`, and the ramp ended in it as §2.1 wrote it: a declaration the index does not name is an error, and the message names no release, because a deadline that has arrived is no longer something a reader can be told in time to act on ([§FS-check.3.18](../../functional-spec/FS-check.md#318-declaration-missing-from-its-kinds-index)). §2.3's three literals are two, the pair [§FS-check.3.17](../../functional-spec/FS-check.md#317-index-entry-is-not-a-link)'s message still carries. The window bought what it was argued for — no repository met the error without having been told the date by every run of the release before it.
