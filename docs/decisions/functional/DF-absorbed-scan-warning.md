...
Two adjacent silent-scope holes stay open and are not this decision's: a node excluded by `include_root = false` whose own files are then scanned by nobody, and a `[workspace]` block no enclosing block lists ([§FS-workspace.6.1](../../functional-spec/FS-workspace.md#61-nested-workspaces)).
