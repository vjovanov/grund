...
Report every `# <KIND>-…: <title>` heading whose ID fails `[id] format`, at its own `path:line`. This is the near miss [§FS-check.5](../../functional-spec/FS-check.md#5-what-grund-does-not-check) already records as unflagged, and it is tracked as [§RM-declaration-near-miss](../../roadmap.md#rm-declaration-near-miss-warn-on-a-heading-that-looks-like-a-declaration-but-does-not-match-id-format).
