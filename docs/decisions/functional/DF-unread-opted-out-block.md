...
The warning fires only where the block's own tree actually holds a file a scan would have read, probed as [§FS-check.4.11](../../functional-spec/FS-check.md#411-include_root--false-leaves-the-blocks-own-files-unread) states it and stopping at the first hit.
...
| [§FS-check.4.11](../../functional-spec/FS-check.md#411-include_root--false-leaves-the-blocks-own-files-unread) an opted-out block's unread files | **no** — a grouping directory with a README it does not need checked is correct | **none** | permanent |
