...
|---|---|---|---|
| [§FS-check.3.18](../../functional-spec/FS-check.md#318-declaration-missing-from-its-kinds-index) declaration missing from its index | yes | `index = false` on the kind | ramps ([§REQ-backwards-compatibility.2](../../requirements/REQ-backwards-compatibility.md#2-the-deprecation-path)) |
...
| [§FS-check.4.7](../../functional-spec/FS-check.md#47-a-workspace-member-swallows-the-blocks-own-scan) a member swallows the block's scan | yes — it claims to be a project and reads nothing | `include_root = false` | ramps |
| [§FS-check.4.8](../../functional-spec/FS-check.md#48-unlisted-workspace-block) unlisted `[workspace]` block | yes — its projects are spelled two ways | list it, or keep it out of the enclosing `[scan]` | ramps |
