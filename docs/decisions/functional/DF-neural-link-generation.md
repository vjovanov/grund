...
| 7 | Terminal TUI (assistant message) | Markdown link over an editor URL | editor | click opens the file in the editor | specimens emitted 2026-07-02, click-through **resolved 2026-08-11** and it is surface-specific: works in Claude Code (row 13), not clickable in the Codex TUI (row 15). Still machine-local by nature — the scheme handler is the user's — which is why the target is a user-configuration key and never committable (§DF-conversation-link-target.2.3) |
