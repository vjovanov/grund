...
- Two further verdict changes, both from `0` to `2`, both in workspaces: a member's unreadable file now fails the run at the root, and so does the root's when a member was the reason the root was loaded. In both cases exit `2` is the code `check`, `list`, and `refs` already return for the same tree, so the change makes two installs of `grund` agree rather than making them differ ([§FS-non-goals.13](../../functional-spec/FS-non-goals.md#13-anything-that-would-let-two-grund-installs-disagree)).
  ...
  **Which clause licenses the move.** Not [§REQ-backwards-compatibility.1](../../requirements/REQ-backwards-compatibility.md#1-what-is-covered)'s enumeration: that section routes a verdict move through the deprecation path (§2) or a loud mechanical migration (§3), and this is neither — there is no old form to carry beside a new one, and no command the tool ships that completes the change. Nor [§4](../../requirements/REQ-backwards-compatibility.md#4-what-was-never-a-promise), whose two items are a construct with *no defined meaning that produced no output*, and the pre-`1.0` rename licence. Neither fits: the old run produced output, and exit `0` had a defined meaning — [§FS-cover.4](../../functional-spec/FS-cover.md#4-exit-codes) states it. What was wrong is that the answer was false, and a false answer is a bug, not an undefined one.

  What licenses it is the requirement's own lead: *"a verdict may move, but only where the release says it will and the finding says what to do."* Both hold. The release says it will — that is what the changelog entry is for. And every finding that replaces the old `0` says what to do at a location: `duplicate workspace project alias \`api\`` names the two `members` entries that collide, a broken symlink names the path that cannot be read. The move is the opposite of the silence the requirement guards against ([§GOAL-no-silent-breakage](../../goals.md#goal-no-silent-breakage-changes-ship-through-a-deprecation-path)).

  Worth recording for whoever next edits the requirement: §1's enumeration is narrower than the lead it elaborates, and it has no slot for a verdict that another hard requirement forbade the tool to produce — which is what this one was ([§REQ-no-missed-citation.1](../../requirements/REQ-no-missed-citation.md#1-no-silent-skips)). Widening §1 or §4 to name that case is a requirements change, not a decision this record can make.
