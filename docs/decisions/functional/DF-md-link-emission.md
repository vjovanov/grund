...
- Repos that keep the default should run `grund fmt --write` as a pre-commit hook so the generated links stay in sync with file moves and citation edits. CI can flag the same link drift without writing by running plain `grund fmt --check`: `enabled = true` auto-runs the link pass in both modes, so the dry run previews exactly what the writer applies.
