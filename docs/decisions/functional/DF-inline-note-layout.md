...
Both are fixed by narrowing *which* lines the rule speaks about rather than by loosening the form (§2.3). A leading list marker is skipped with the indentation, and a citation-bearing line is judged only when it is the site's first one or when it opens with a citation token — a line that opens with prose and names an ID later is the continuation of a note that already opened correctly. What the narrowing costs is stated rather than hidden: a citation written prose-first on a continuation line is not reported, so the rule is a check on how notes *open*, not a guarantee that every `§<ID>` in a block sits in front of a colon. Judging those lines instead would need to know where a sentence ends, which is the judgment [§FS-non-goals.3](../../functional-spec/FS-non-goals.md#3-code-ast-parsing) refuses to acquire — the same reason `grund fmt` stays out of this rule entirely (§2.5). The first citation-bearing line stays judged unconditionally, because that is the line that opens the note and the one form worth demanding is how a note opens.
...
Exact is about the form, never about the window it is read in. Stripping a comment prefix, the indentation behind it, and a leading list marker (§2.2) decides *where the content starts*; requiring `, ` between two citations and a space after the colon decides *what the content must say*. Relaxing the first is how the rule stays a statement about layout rather than about comment formatting; relaxing the second would hand the project back the variation it configured the key to remove.
...
| Judge every citation-bearing line, continuations included | Makes a wrapped note and a second cited point mutually exclusive, so `inline_note_max_lines` and rule 3 contradict each other as soon as both are used; the author's only fix is to reword the sentence. §2.2. |
