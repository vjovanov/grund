...
- [§FS-config.3.10](functional-spec/FS-config.md#310-renderlinks--clickable-citation-rendering): add the single `[render.links].conversation = "plain" | "link"` policy so repositories choose whether agents write bare or linked conversation citations. Link targets come from the writing context rather than repo-wide configuration. Additive, optional, and validated by `grund config validate`; `grund config show` displays the effective value. PR #45.
