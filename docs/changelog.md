...
- [§DF-neural-link-generation](decisions/functional/DF-neural-link-generation.md#df-neural-link-generation-agents-compose-clickable-citation-links-themselves-grund-does-not-grow-a-link-command): record the clickable-citations experiment and decision for conversation text as an agent convention, not a tool. Repositories choose plain citations or context-valid links through `[render.links].conversation`; links keep the citation as visible text and plain is always the fallback. Rendering-layer integrations make the plain form clickable locally. `grund` grows no `link` command (a prototype was built and reverted on the PR branch); `grund fmt --cross-refs` ([§FS-fmt.6](functional-spec/FS-fmt.md#6-cross-reference-emission)) remains the only link emitter for repository Markdown. PR #45.
- [§FS-integrations](functional-spec/FS-integrations.md#fs-integrations-grund-prints-and-installs-its-rendering-layer-integrations) / [§DF-integrations-command](decisions/functional/DF-integrations-command.md#df-integrations-command-integrations-earns-a-cli-slot-as-one-time-setup-where-a-per-citation-link-command-did-not): add the `grund integrations [<client>]` command — the one-stop shop that makes the clickable-citation rendering layer installable from `cargo install grund`. It prints embedded artifacts on demand (a config snippet plus the `grund-open` resolver for `kitty`, `tmux`, `wezterm`; the unpacked terminal-citations extension for `vscode`), detects the caller's terminal/editor from the environment when given no client, and installs on `--write` as an idempotent, marked block — the `completions`/`init` ethos. `--format json` emits a machine detection plan for agents. Every artifact is embedded in the binary; nothing is published to a marketplace, preserving the no-first-party-plugin stance ([§FS-lsp.2.3](functional-spec/FS-lsp.md#23-editor-configuration-one-time-per-editor)). Resolves issue #46. PR #45.
- [§FS-config.3.10](functional-spec/FS-config.md#310-renderlinks--clickable-citation-rendering): add the `[render.links]` config table (`conversation = "plain" | "link"`, plus `web_base` and `hover_title`) so repositories choose whether agents write bare or linked conversation citations. Link targets come from the writing context rather than repo-wide ref configuration. Additive and optional; every key carries a default and is validated on load. `grund config validate` / `grund config show` cover it. PR #45.

### Changed

- **Schema:** [§FS-init.2.3.6](functional-spec/FS-init.md#236-clickable-citations): the `AGENTS.md` init block version is **v3 → v4**. The block now carries a generated `### Clickable citations` section rendered from `[render.links]` ([§FS-config.3.10](functional-spec/FS-config.md#310-renderlinks--clickable-citation-rendering)), drift-checked by `grund check` the same way `### Citation directions` is. Migration: run `grund init` in each repo to refresh the managed block to v4; `grund check` flags a v3 block as outdated until then. PR #45.
