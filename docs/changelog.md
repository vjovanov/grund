...
- [§FS-config.3.10](functional-spec/FS-config.md#310-renderlinks--clickable-citation-rendering): add the `[render.links]` config table (`web_base`, `web_ref`, `hover_title`, `local`) so the clickable-citations convention an agent follows is a rendering of repository config rather than hand-edited prose. Additive and optional; every key carries a default and is validated on load. `grund config validate` / `grund config show` cover it. PR #45.

### Changed

- **Schema:** [§FS-init.2.3.6](functional-spec/FS-init.md#236-clickable-citations): the `AGENTS.md` init block version is **v3 → v4**. The block now carries a generated `### Clickable citations in user-facing text` section rendered from `[render.links]` ([§FS-config.3.10](functional-spec/FS-config.md#310-renderlinks--clickable-citation-rendering)), drift-checked by `grund check` the same way `### Citation directions` is. Migration: run `grund init` in each repo to refresh the managed block to v4; `grund check` flags a v3 block as outdated until then. PR #45.
