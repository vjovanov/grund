...
When the effective `.agents/grund.toml` sets `[reference] conversation = "link"` ([§FS-config.3.1](FS-config.md#31-reference--citation-form), [§DF-repo-conversation-opinion](../decisions/functional/DF-repo-conversation-opinion.md#df-repo-conversation-opinion-repositories-may-commit-a-link-only-conversation-rendering-opinion)), the section additionally carries the config-derived sentences: `In local conversations, follow §<ID> with its declaration location as plain path:line text — never a Markdown link; fall back to the bare citation when unsure. If a user-level grund block asks for plain citations, write bare citations instead: that machine renders them clickable itself.` The no-Markdown-link clause is bound to the local-conversation sentence with an em-dash, matching the [§FS-integrations.4.3](FS-integrations.md#43-user-preference-and-global-agent-instructions) `link` block: a detached final sentence would read as revoking the repository-web sentence, which requires a link. The deference clause is what keeps rendering machine-local ([§DF-repo-conversation-opinion.2.3](../decisions/functional/DF-repo-conversation-opinion.md#23-precedence)): the committed opinion is the fallback for machines that never stated a preference — fresh clones, cloud and CI sessions, agents whose only instruction channel is the committed entrypoint — while a machine that recorded `plain` through `grund integrations --write` has evidence of an installed rendering layer, and its preference wins. When the key is absent, the sentence is absent, and local conversation rendering is governed solely by the user preference `grund integrations --write` installs in user-level agent instructions ([§FS-integrations.4.3](FS-integrations.md#43-user-preference-and-global-agent-instructions)). The full recipe, rationale, and test matrix stay in the cited decision; the repository block carries only the compact, forge-neutral convention.
...
The managed block renders a `### Clickable citations` section carrying the content points in §2.3.4.17: the fixed repository-web sentence always, plus the local-conversation sentence when `[reference] conversation = "link"` is set. The section is deterministic from the effective config ([§FS-non-goals.13](FS-non-goals.md#13-anything-that-would-let-two-grund-installs-disagree)): byte-identical in every project with the same key state. Adding the fixed sentence bumped the managed-block version to **v4**; adding the config-derived local-conversation sentence bumps it to **v5**, both carried under [§GOAL-no-silent-breakage](../goals.md#goal-no-silent-breakage-changes-ship-through-a-deprecation-path); an older supported block is reported outdated by `grund check` until `grund init` re-renders it. Because the local-conversation sentence derives from config, the section joins the citation-directions drift check (§2.3.5): `grund check` re-renders it from the live config and byte-compares; a mismatch — including flipping the `conversation` key without re-running `grund init` — is an `agents-init` finding ([§FS-check.3.5](FS-check.md#35-invalid-agent-entrypoint-init-block)).
