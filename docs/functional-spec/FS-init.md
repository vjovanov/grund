...
The block teaches how a `§<ID>` citation renders as a clickable link in ephemeral, user-facing text — agent TUI messages and GitHub PR/issue bodies — as a rendering of the effective `[render.links]` config ([§FS-config.3.10](FS-config.md#310-renderlinks--clickable-citation-rendering)). It tells agents: in local TUI messages write the citation in the form named by `local` (a plain `§<ID>` by default, which an installed integration resolves — [§FS-integrations](FS-integrations.md#fs-integrations-grund-prints-and-installs-its-rendering-layer-integrations)); on GitHub surfaces compose a blob-URL link from `web_base`/`web_ref`, optionally carrying the declaration heading as the link title when `hover_title` is set; and always fall back to the plain citation when unsure, never rewriting repository files by hand ([§DF-neural-link-generation](../decisions/functional/DF-neural-link-generation.md#df-neural-link-generation-agents-compose-clickable-citation-links-themselves-grund-does-not-grow-a-link-command)). The full recipe, rationale, and test matrix stay in the cited decision; the block carries only the two-sentence convention.
...
The managed block renders a `### Clickable citations in user-facing text` section from the effective `[render.links]` config ([§FS-config.3.10](FS-config.md#310-renderlinks--clickable-citation-rendering)), carrying the content point in §2.3.4.17. Rendering is deterministic: the `local` value selects one of the fixed local-form phrases, and `web_base`, `web_ref`, and `hover_title` select the fixed GitHub-form phrases — so the same effective config produces byte-identical text ([§FS-non-goals.13](FS-non-goals.md#13-anything-that-would-let-two-grund-installs-disagree)). A config with no `[render.links]` table renders the same section from the defaults, so every repo carries the convention.
