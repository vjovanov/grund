...
The block carries only the fixed repository-scoped sentence: `On repository web surfaces, link §<ID> to the PR branch in PR bodies, the reviewed commit in reviews, an exact commit for permalinks, and the default branch otherwise; fall back to plain when unsure.` ([§DF-neural-link-generation](../decisions/functional/DF-neural-link-generation.md#df-neural-link-generation-agents-compose-clickable-citation-links-themselves-grund-does-not-grow-a-link-command)). Local conversation rendering depends on the user's TUI setup, so `grund integrations --write` installs that preference in user-level agent instructions instead ([§FS-integrations.4.3](FS-integrations.md#43-user-preference-and-global-agent-instructions)). The full recipe, rationale, and test matrix stay in the cited decision; the repository block carries only the compact, forge-neutral convention.
...
The managed block renders a fixed `### Clickable citations` section carrying the content point in §2.3.4.17. It is independent of repository configuration and therefore byte-identical in every project ([§FS-non-goals.13](FS-non-goals.md#13-anything-that-would-let-two-grund-installs-disagree)). Adding the section is the content change that bumps the managed-block version to **v4**, carried under [§GOAL-no-silent-breakage](../goals.md#goal-no-silent-breakage-changes-ship-through-a-deprecation-path); a v3 block is reported outdated by `grund check` until `grund init` re-renders it. Unlike the config-derived citation-directions section (§2.3.5), it needs no separate drift check beyond normal managed-block rendering.
