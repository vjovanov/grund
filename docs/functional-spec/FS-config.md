...
The block carries repository truths about how a `§<ID>` citation renders, so the convention an agent follows is generated rather than hand-edited ([§FS-init.2.3.6](FS-init.md#236-clickable-citations)). `conversation` is the only policy: `plain` writes the bare citation and lets an installed integration ([§FS-integrations](FS-integrations.md#fs-integrations-grund-prints-and-installs-its-rendering-layer-integrations)) resolve it; `link` wraps the citation in a context-valid declaration link and falls back to plain when uncertain. `web_base` supplies the blob-URL prefix when that link is web-shaped, and `hover_title` toggles its heading title. The ref is deliberately not configuration: the writing context supplies it. The user's editor choice also remains outside shared repository config; it lives in the installed integration or `GRUND_OPEN_CMD` ([§DF-neural-link-generation](../decisions/functional/DF-neural-link-generation.md#df-neural-link-generation-agents-compose-clickable-citation-links-themselves-grund-does-not-grow-a-link-command)).

The table is optional; without it every key takes its default and citations render exactly as the shipped convention describes. Every key is enum- or shape-validated on load: `web_base` is exactly `auto` or a non-empty `https://` URL prefix safe to embed in a Markdown link, and an unknown key under `[render.links]` is an error like any other config typo (§4.3). Adding `[render.links]` does **not** bump `grund_config_version` (§5): it is additive surface like `[workspace]`, `[citations]`, and `[fmt.cross_refs]`. An older binary meeting it fails loudly with `unknown config section`.
