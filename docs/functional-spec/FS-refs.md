...
- `<ID>` — the ID to look up, without the marker. May carry an inline section (`FS-check.3.1`, or `FS-plan.goals.performance` when named sections are enabled) using the configured `[id] section_separator`; equivalently pass `--section 3.1` or `--section goals.performance`. Parsing uses the named kind's `[[kinds]].format` override when present and `[id].format` otherwise ([§FS-config.3.2](FS-config.md#32-id--id-grammar)). An argument outside its effective format is rejected before the scan with the ordinary invalid-ID hint, which names that effective format. The number-only shorthand is accepted where the effective format has one — `grund refs FS-042` lists the citations of `FS-042-user-login`, including any written in the shorthand themselves ([§FS-check.1.2](FS-check.md#12-the-number-only-shorthand)) — and a shorthand matching more than one declaration is rejected with the candidate list rather than resolved to a guess.
