...
**`include_root = false` changes nothing.** The block still contributes a segment to every alias path below it ([§FS-workspace.6.1](FS-workspace.md#61-nested-workspaces)), so the two scopes still disagree about how the projects under it are spelled. Same finding, same message.
