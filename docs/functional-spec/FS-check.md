...
Like §2.2 it is a warning, and like §2.2 it is withheld from a run that has any other finding: the exit code stays `0` (a tree with nothing in it yet is the ordinary first day of a repository), and a report that already says something is not the silent verdict this rule exists to break. The redundant-config caution (§4.3) is the one exception §2.2 also makes — the two are independent facts about the run, and a repository mid-migration between the two config names is exactly where a mismatched `[id] format` hides, in the file that is no longer read. What it buys is the `success` marker — a warning stands in its place (§2.1), so the run that recognized nothing stops printing the same word as the run that checked everything.
