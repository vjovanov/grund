...
**Only the outermost block of a chain.** A `[workspace]` block below an unlisted one *is* claimed — by the unlisted block — so the claim test answers it on its own, and listing the outer block puts the whole chain back in the claimed chain. One finding for one edit. Two unlisted blocks neither of which lists the other are two findings, because they are two edits.
