...
Both the click matchers and the resolver therefore accept a bare local `§<ID>` and a namespace-qualified `§<alias>/<ID>`; in a workspace the qualified form resolves through `grund`'s own alias routing ([§FS-workspace.8.1](FS-workspace.md#81-grund-)) rather than through string matching over the ID catalog.

Two WezTerm facts are part of the contract because each failure is silent. The click and peek gestures are registered twice — once plainly and once with `mouse_reporting = true` — because WezTerm ignores user mouse bindings while the foreground program has captured the mouse, and the programs that print citations (agent TUIs like Claude Code, full-screen editors) are exactly such programs; without the second registration the primary use case never clicks. And the resolver spawn goes through `sh` with `~/.local/bin` and `~/.cargo/bin` prepended to `PATH` — the documented homes of `grund-open` (§4.1) and `grund` — because a desktop-launched GUI never sourced the user's shell profile; when WezTerm itself runs inside a Flatpak sandbox (detected via `FLATPAK_ID`), whose `PATH` carries neither binary, the spawn is handed back to the host through `flatpak-spawn --host`.
