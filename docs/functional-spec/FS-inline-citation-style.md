...
The sentence names a house style for the comments an agent writes, and it is deliberately wider than the gate: `check` judges inline citation *sites*, so a doc-comment that declares an ID is not measured against the form at all (§3.3, rule 6). Both readings are the intended ones — an agent should lay out every note it writes the same way, and a declaration body is spec text whose shape this spec does not govern. The practical consequence belongs to whoever migrates a tree: under `warn`, the worklist covers the citing comments and never the declaration bodies, so "the report is empty" means the sites are clean, not that every `§<ID>` line in the repository is citation-first.
