...
`check`, ID queries, `refs`, `list`, `cover`, `config show` / `validate`, `completions`, `agent-setup-instructions`, LSP requests, and `integrations` without `--write` open files read-only. No caches, no lock files, no PID files, no "fixed it for you" rewrites — their only output is the streams ([§FS-errors.1](../functional-spec/FS-errors.md#1-streams)).

## 2. Writers touch only what they own

Four invocations write, each bounded by a stated ownership marker: `fmt --write` rewrites the citation tokens it names and no other byte ([§FS-fmt.2.3](../functional-spec/FS-fmt.md#23-what-is-never-rewritten)); `fetch` atomically replaces or inserts only its validated named declaration in the configured snapshot home ([§FS-fetch.6](../functional-spec/FS-fetch.md#6-stability-and-ownership)); `init` owns the managed entrypoint block, delimited by its markers, plus the files it scaffolds ([§FS-init.3](../functional-spec/FS-init.md#3-non-intrusive-guarantees)); `integrations --write` owns its comment-delimited blocks in editor and terminal config, its fixed resolver path, and its own extension directory ([§FS-integrations.4](../functional-spec/FS-integrations.md#4-managed-writes---write)). The editor transform is held to the same rule in the user's buffer ([§FS-lsp.1.4](../functional-spec/FS-lsp.md#14-live-trigger-transform)). Ownership is what makes a re-run safe: everything outside the marker survives, however many times the command runs.
