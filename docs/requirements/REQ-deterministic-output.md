...
`tests/integration/scan_determinism.rs` holds the one input this page does not name — the thread count: every report over this repository and over every plain-check fixture is byte-identical at one thread and at eight.
