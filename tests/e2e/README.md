...
- a workspace member the config declares may be legitimately absent ([§FS-workspace.2.2](../../docs/functional-spec/FS-workspace.md#22-a-member-that-may-be-legitimately-absent), [grund#85](https://github.com/vjovanov/grund/issues/85)): the ticket's own repro with the member present and absent, the absent run naming the unverified namespace on **stdout** and standing in place of `success` while the citation into it goes unreported ([§FS-check.4.10](../../docs/functional-spec/FS-check.md#410-a-workspace-member-declared-optional-is-absent)) — with the four shapes that hold the default and the alias rule in place beside it: a member absent from plain `members` still fatal and now naming the key, a present member whose `project_name` disagrees with its entry's last segment, a trailing `/*` refused with the shape to write instead, and a nested block's own absent optional member reported at its own line
- a `fmt` run narrowed below a workspace root, walking a symlink out of the root project's scanned tree ([§FS-workspace.6](../../docs/functional-spec/FS-workspace.md#6-nested-project-boundary)): a link into a member is the boundary, so `--write` leaves that file and the member's own copy of it byte-identical, while a link into a directory no project owns is not a boundary and is rewritten like any other file the walk reads — the pair is what tells the boundary apart from symlinks in general
- `include_root = false` leaving the excluded root's own files scanned by nobody: a dangling citation there passes even `check --full`, and the run now says so ([§FS-check.4.11](../../docs/functional-spec/FS-check.md#411-include_root--false-leaves-the-blocks-own-files-unread), [grund#71](https://github.com/vjovanov/grund/issues/71)) — the ticket's own nested tree under `check`, `check --full`, `list` and `--format json`, the single-level root the ticket calls pre-existing in kind, a run rooted at the opted-out block itself, and two opted-out blocks in one tree earning one line each at their own `include_root` line
